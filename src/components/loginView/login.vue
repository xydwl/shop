			...
			<div class="loginCenter">
				...
				<div class="login">
					<ul class="loginUl">
						...
						<li :class="{active:loginway==='register'}" class="loginUlLi2" @click="changeLoginway('register')">注册</li>
					</ul>
					<div class="loginDiv" v-if="loginway ==='login'">
						...
						<p>
							<input type="text" class="inputInt" placeholder="验证码" v-model="picCode" id="picCodeImgLoginInt" />
							<input type="text" id="verificationCodeTempLogin" v-model="loginCode" />
							<img :src="src" class="abcd" id="picCodeImgLogin" @click="getCaptchaCode" />
						</p>
						<div class="cl"></div>
						<p class="loginLog" @click="subLogin">立即登录</p>
						<p class="loginMima">
							<router-link :to="{name:'forgetPsd'}">忘记密码?</router-link>
						</p>
					</div>
					<!--注册-->
					<div class="registerDiv" v-if="loginway==='register'">
						<alert-tip :alertTips="alertTips" v-show="tipShow"></alert-tip>
						<p class="loginTel">
							<input type="text" v-model="registerUserName" placeholder="请输入手机号" style="text-indent: 5px;" id="registerUserName" />
						</p>
						<p class="loginTel">
							<input type="password" v-model="registerPassword" placeholder="设置密码 : 长度为6~16位字符" style="text-indent: 5px;" id="registerPassword" />
						</p>
						<p class="loginTel">
							<input type="password" v-model="repassword" placeholder="确认密码" style="text-indent: 5px;" id="repassword" />
						</p>
						<p style="margin-bottom: 20px; height: 62px;">
							<input type="text" class="inputInt" v-model="picCode" placeholder="验证码" id="picCode" />
						...
						<p class="registerCheck">
							<input type="checkbox" v-model="checked" id="agreeCheck" />我已阅读并同意
							<a href="javascript:;" @click="showXy">
								《拍库用户注册协议》
							</a>
						</p>
						<p class="loginLog" style="margin-top:0px; margin-bottom: 0;" @click="getRegisterData">立即注册</p>
					</div>
				</div>
			</div>
		</div>
		<!--遮罩-->
		<div class="addressShade" v-show="mark"></div>
		<!-- 协议弹出层 -->
		<div class="xieyi" v-show="xieyiMark">
			<div class="xieyiHeader">
				pecoo用户注册协议
			</div>
			<span class="close" onclick="closeBox()"></span>
			<div class="xieyiContent">
				<p>本协议是您与拍库网站（简称"本站"，网址：www.pecoo.com）所有者（以下简称为"拍库"）之间就拍库网站服务等相关事宜所订立的契约，请您仔细阅读本注册协议，您点击"同意并继续"按钮后，本协议即构成对双方有约束力的法律文件。</p>
				<p class="title">第1条 本站服务条款的确认和接纳</p>
				<p>1.1本站的各项电子服务的所有权和运作权归拍库所有。用户同意所有注册协议条款并完成注册程序，才能成为本站的正式用户。用户确认：本协议条款是处理双方权利义务的契约，始终有效，法律另有强制性规定或双方另有特别约定的，依其规定。</p>
				<p>1.2用户点击同意本协议的，即视为用户确认自己具有享受本站服务、下单购物等相应的权利能力和行为能力，能够独立承担法律责任。</p>
				<p>1.3如果您在18周岁以下，您只能在父母或监护人的监护参与下才能使用本站。</p>
				<p>1.4拍库保留在中华人民共和国大陆地区法施行之法律允许的范围内独自决定拒绝服务、关闭用户账户、清除或编辑内容或取消订单的权利。</p>
				<p class="title">第2条 本站服务</p>
				<p>2.1拍库通过互联网依法为用户提供互联网信息等服务，用户在完全同意本协议及本站规定的情况下，方有权使用本站的相关服务。</p>
				<p>2.2用户必须自行准备如下设备和承担如下开支：</p>
				<p>（1）上网设备，包括并不限于电脑或者其他上网终端、调制解调器及其他必备的上网装置；</p>
				<p>（2）上网开支，包括并不限于网络接入费、上网设备租用费、手机流量费等。</p>
				<p class="title">第3条 用户信息</p>
				<p>3.1用户应自行诚信向本站提供注册资料，用户同意其提供的注册资料真实、准确、完整、合法有效，用户注册资料如有变动的，应及时更新其注册资料。如果用户提供的注册资料不合法、不真实、不准确、不详尽的，用户需承担因此引起的相应责任及后果，并且拍库保留终止用户使用拍库各项服务的权利。</p>
				...
				<p>3.4用户同意，拍库拥有通过邮件、短信电话等形式，向在本站注册、购物用户、收货人发送订单信息、促销活动等告知信息的权利。</p>
				<p>3.5用户不得将在本站注册获得的账户借给他人使用，否则用户应承担由此产生的全部责任，并与实际使用人承担连带责任。</p>
				<p>3.6用户同意，拍库有权使用用户的注册信息、用户名、密码等信息，登录进入用户的注册账户，进行证据保全，包括但不限于公证、见证等。</p>
				<p class="title">第4条 用户依法言行义务</p>
				...
				<p class="title">第5条 商品信息</p>
				<p>本站上的商品价格、数量、是否有货等商品信息随时都有可能发生变动，本站不作特别通知。由于网站上商品信息的数量极其庞大，虽然本站会尽最大努力保证您所浏览商品信息的准确性，但由于众所周知的互联网技术因素等客观原因存在，本站网页显示的信息可能会有一定的滞后性或差错，对此情形您知悉并理解；拍库欢迎纠错。</p>
				<p>为表述便利，商品和服务简称为"商品"或"货物"。</p>
				<p class="title">第6条 订单</p>
				...
				<p>6.2除法律另有强制性规定外，双方约定如下：本站上销售方展示的商品和价格等信息仅仅是交易信息的发布，您下单时须填写您希望购买的商品数量、价款及支付方式、收货人、联系方式、收货地址等内容；系统生成的订单信息是计算机信息系统根据您填写的内容自动生成的数据，仅是您向销售方发出的交易诉求；销售方收到您的订单信息后，只有在销售方将您在订单中订购的商品从仓库实际直接向您发出时（ 以商品出库为标志），方视为您与销售方之间就实际直接向您发出的商品建立了交易关系；只有在销售方实际直接向您发出了订单中订购的其他商品时，您和销售方之间就订单中该其他已实际直接向您发出的商品才成立交易关系。您可以随时登录您在本站注册的账户，查询您的订单状态。</p>
				<p>6.3由于市场变化及各种以合理商业努力难以控制的因素的影响，本站无法保证您提交的订单信息中希望购买的商品都会有货；如您拟购买的商品，发生缺货，您有权取消订单。</p>
				<p class="title">第7条 配送</p>
				...
				<p class="title">第8条 所有权及知识产权条款</p>
				<p>8.1用户一旦接受本协议，即表明该用户主动将其在任何时间段在本站发表的任何形式的信息内容的财产性权利等任何可转让的权利，如著作权财产权（包括并不限于：复制权、发行权、出租权、展览权、表演权、放映权、广播权、信息网络传播权、摄制权、改编权、翻译权、汇编权以及应当由著作权人享有的其他可转让权利），全部独家且不可撤销地转让给拍库所有，用户同意京东有权就任何主体侵权而单独提起诉讼。</p>
				<p>8.2本协议已经构成《中华人民共和国著作权法》第二十五条（条文序号依照2011年版著作权法确定）及相关法律规定的著作财产权等权利转让书面协议，其效力及于用户在拍库网站上发布的任何受著作权法保护的作品内容，无论该等内容形成于本协议订立前还是本协议订立后。</p>
				<p>8.3用户同意并已充分了解本协议的条款，承诺不将已发表于本站的信息，以任何形式发布或授权其它主体以任何方式使用（包括但不限于在各类网站、媒体上使用）。</p>
				<p>8.4拍库是本站的制作者,拥有此网站内容及资源的著作权等合法权利,受国家法律保护,有权不时地对本协议及本站的内容进行修改，并在本站张贴，无须另行通知用户。在法律允许的最大限度范围内，拍库对本协议及本站内容拥有解释权。</p>
				<p>8.5除法律另有强制性规定外，未经拍库明确的特别书面许可,任何单位或个人不得以任何方式非法地全部或部分复制、转载、引用、链接、抓取或以其他方式使用本站的信息内容，否则，京东有权追究其法律责任。</p>
				<p>8.6本站所刊登的资料信息（诸如文字、图表、标识、按钮图标、图像、声音文件片段、数字下载、数据编辑和软件），均是拍库或其内容提供者的财产，受中国和国际版权法的保护。本站上所有内容的汇编是拍库的排他财产，受中国和国际版权法的保护。本站上所有软件都是拍库或其关联公司或其软件供应商的财产，受中国和国际版权法的保护。</p>
				<p class="title">第9条 责任限制及不承诺担保</p>
				<p>除非另有明确的书面说明,本站及其所包含的或以其它方式通过本站提供给您的全部信息、内容、材料、产品（包括软件）和服务，均是在"按现状"和"按现有"的基础上提供的。</p>
				<p>除非另有明确的书面说明,拍库不对本站的运营及其包含在本网站上的信息、内容、材料、产品（包括软件）或服务作任何形式的、明示或默示的声明或担保（根据中华人民共和国法律另有规定的以外）。</p>
				<p>拍库不担保本站所包含的或以其它方式通过本站提供给您的全部信息、内容、材料、产品（包括软件）和服务、其服务器或从本站发出的电子信件、信息没有病毒或其他有害成分。</p>
				<p>如因不可抗力或其它本站无法控制的原因使本站销售系统崩溃或无法正常使用导致网上交易无法完成或丢失有关的信息、记录等，京东会合理地尽力协助处理善后事宜。</p>
				<p class="title">第10条 协议更新及用户关注义务</p>
				<p>根据国家法律法规变化及网站运营需要，拍库有权对本协议条款不时地进行修改，修改后的协议一旦被张贴在本站上即生效，并代替原来的协议。用户可随时登录查阅最新协议；用户有义务不时关注并阅读最新版的协议及网站公告。如用户不同意更新后的协议，可以且应立即停止接受拍库网站依据本协议提供的服务；如用户继续使用本网站提供的服务的，即视为同意更新后的协议。拍库建议您在使用本站之前阅读本协议及本站的公告。 如果本协议中任何一条被视为废止、无效或因任何理由不可执行，该条应视为可分的且并不影响任何其余条款的有效性和可执行性。</p>
				<p class="title">第11条 法律管辖和适用</p>
				...
				<p class="title">第12条 其他</p>
				<p>12.1拍库网站所有者是指在政府部门依法许可或备案的拍库网站经营主体。</p>
				<p>12.2拍库尊重用户和消费者的合法权利，本协议及本网站上发布的各类规则、声明等其他内容，均是为了更好的、更加便利的为用户和消费者提供服务。本站欢迎用户和社会各界提出意见和建议，拍库将虚心接受并适时修改本协议及本站上的各类规则。</p>
				<p>12.3本协议内容中以黑体、加粗、下划线、斜体等方式显著标识的条款，请用户着重阅读。</p>
				<p>12.4您点击本协议下方的"同意"按钮即视为您完全接受本协议，在点击之前请您再次确认已知悉并完全理解本协议的全部内容。</p>
			</div>
			<div class="xieyiBtn">
				<button class="sureBtn" type="button" value="同意" @click="closeBox">同意</button>
			</div>
		</div>
		<!-- 注册成功跳转弹出层 -->
		<div id="registerSuc" class="loginSuc" v-show="resSuccess">
			<div>恭喜您成功注册成为拍库用户</div>
			<p class="loginSucCen">拍库小助手为您准备了一次首单免费试拍的机会，赶紧去开始拍宝之旅吧！您也可以选择充值保证金，保证金使用规则为充值2000元可以拍买3件拍品，保证金的具体使用规则你可以咨询在线客服，或者拨打拍库热线400-111-2016，如果您选择充值保证金，则视为您自动放弃免费试拍资格。</p>
			<p style="text-align:center">
				<a href="http://www.pecoo.com" role="button" class="loginSucPai">去拍宝</a>
				<a href="http://www.pecoo.com/member/pay/toselectPayWays?type=1&paidMoney=2000" role="button" class="loginSucPai">去充值</a>
			</p>
		</div>
...
export default {
  ...
  computed: {
    loginway () {
      return this.$store.getters.getLoginway
    }
  },
  components: {
    footBottom,
